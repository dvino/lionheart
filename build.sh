...
build_golib() {
    ...
    gomobile bind -target=android -androidapi=24 -ldflags="-s -w" -o "$ANDROID/app/libs/liblionheart.aar" .
    echo -e "  ${GREEN}✓ .aar${NC}"
    cd "$ROOT"
}
build_cli() {
...
build_debugapk() {
    echo -e "\n${BOLD}[APK] Android (debug only)...${NC}"
    check_go
    find_android_sdk
    if ! find_ndk; then
    ...
    sync_config
    local ANDROID="$ROOT/mobile/android"
    # Пересобираем .aar только если его нет или Go-код изменился
    local AAR="$ANDROID/app/libs/liblionheart.aar"
    local NEED_AAR=0
    if [ ! -f "$AAR" ]; then
        NEED_AAR=1
    else
        # Проверяем, изменились ли Go-файлы после последней сборки .aar
...
case "$MODE" in
    ...
    *)        echo -e "Использование: ./build.sh [cli|apk|debugapk|all]"
              echo ""
              echo "  cli       — только CLI бинарники (все платформы)"
              echo "  debugapk  — только debug APK + adb install (быстро)"
              echo "  apk       — debug + release APK"
              echo "  all       — всё (CLI + APK)"
              exit 1 ;;
esac
echo ""
echo -e "${GREEN}═══════════════════════════════════════${NC}"
